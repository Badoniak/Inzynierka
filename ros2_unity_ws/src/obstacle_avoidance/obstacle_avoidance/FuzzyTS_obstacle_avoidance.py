...
class TSObstacleAvoidance(Node):
    def __init__(self):
        ...
        self.declare_parameter('side_danger_distance', 0.25)
        ...
        self.side_danger_distance = self.get_parameter('side_danger_distance').value
    ...
    def mu_front(self, x):
        # S = naprawdę blisko → panika
        S = triangle(x, -1.0, 0.5, 1.6)

        # M = tylko przejście (NIE steruje mocno)
        M = triangle(x, 1.2, 2.6, 4.3)

        # L = traktuj jak "nie istnieje"
        L = triangle(x, 3.2, 8.0, 30)
        return S, M, L
    def mu_side(self, x):
        # C = bardzo blisko → agresywna reakcja
        C = triangle(x, -1.0, 0.6, 1.6)

        # M = normalna jazda przy przeszkodzie
        M = triangle(x, 1.3, 1.8, 2.8)

        # F = naprawdę wolna przestrzeń
        F = triangle(x, 2.6, 6.0, 30)
        return C, M, F
    # ---------------- RULE TABLE ----------------
    ...
    RULES = {
        ...
        ("M","C","C"):(0.7, -1.3), # Wąsko -> wolno i korekta
        ("M","C","M"):(0.8, -1.3), # Blisko z lewej -> w prawo
        ("M","C","F"):(0.9, -1.4), # Blisko z lewej -> w prawo (szybciej bo F wolne)
        ("M","M","C"):(0.8, +1.3), # Blisko z prawej -> w lewo
        ("M","F","C"):(0.9, +1.4), # Blisko z prawej -> w lewo (szybciej bo F wolne)
        # --- JAZDA KORYTARZEM / ŚREDNIO Z BOKÓW (M) ---
        # Priorytet: Płynność (małe skręty, duża prędkość) - TU BYŁ PROBLEM OSCYLACJI
        ("M","M","M"):(1.2, -0.7), # Jedź prosto
        ("M","M","F"):(1.2, -1.05), # Lekka korekta w prawo
        ("M","F","M"):(1.2, +1.05), # Lekka korekta w lewo
        ("M","F","F"):(1.2, -0.5),

        # Analogicznie dla L (Large Front) - tu możemy jechać szybciej
        ("L","C","C"):(0.85, -0.5), # Jedź środkiem
        ("L","C","M"):(0.65, -1.35), # Korekta w prawo
        ("L","C","F"):(0.75, -1.25), # Korekta w prawo
        ("L","M","C"):(0.65, +1.35), # Korekta w lewo
        ("L","F","C"):(0.75, +1.25), # Korekta w lewo

        
        ("L","M","M"):(1.2, -0.5), # Szybko prosto
        ("L","M","F"):(1.2, -0.9), # Bardzo delikatna korekta (anty-oscylacja)
        ("L","F","M"):(1.2, +0.9), # Bardzo delikatna korekta (anty-oscylacja)

        # --- OTWARTA PRZESTRZEŃ (F) ---
        ("L","F","F"):(1.2, 0.0), # Maksymalna prędkość
    }
...
# HARD SAFETY OVERRIDE — front bardzo blisko
        if front < self.danger_distance or left < self.side_danger_distance or right < self.side_danger_distance :
            msg = Twist()
            # skręcaj w stronę większej przestrzeni
            turn_dir = 1.0 if left > right else -1.0
            msg.linear.x = 0.0
            msg.angular.z = turn_dir * 1.2  # mocniej niż wcześniej
            self.vel_pub.publish(msg)
            self.get_logger().warn("HARD FRONT AVOID — ROTATE IN PLACE")
            return
        ...
        if denominator > 0.0:
            ...
            self.get_logger().info(f"Action: LIN={final_u1:.2f}, ANG={final_u2:.2f}, front={memb_front}, right={memb_right}, left={memb_left}")
        else:
